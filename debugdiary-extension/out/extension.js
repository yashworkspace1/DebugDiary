...
function activate(context) {
    ...
    function updateStatusBar() {
        ...
        statusBar.command = (0, auth_1.isConnected)() ? 'debugdiary.openDashboard' : 'debugdiary.connect';
        statusBar.show();
    }
    updateStatusBar();
    ...
    const connectCmd = vscode.commands.registerCommand('debugdiary.connect', async () => {
        const key = await vscode.window.showInputBox({
            ...
            prompt: `Paste your API key from ${(0, auth_1.getApiUrl)()}/settings/api-keys`,
            password: true,
        ...
        vscode.window.withProgress({
            ...
            await (0, auth_1.setApiKey)(key);
            const result = await (0, api_1.verifyKey)();
            if (result.valid) {
                updateStatusBar();
                vscode.window.showInformationMessage(`✓ DebugDiary connected! Welcome ${result.userName}. Right-click any error to save it.`);
            }
            else {
                await (0, auth_1.setApiKey)('');
                vscode.window.showErrorMessage(`Invalid API key. Check your key at ${(0, auth_1.getApiUrl)()}`);
            }
        });
    });
    ...
    const saveCmd = vscode.commands.registerCommand('debugdiary.saveError', async () => {
        ...
        if (!errorText)
            return;
        const dejavu = await (0, api_1.checkDejavu)(errorText);
        if (dejavu.match) {
            const similarity = Math.round((dejavu.similarity || 0) * 100);
            const action = await vscode.window.showWarningMessage(`⚡ Déjà Vu! You hit something similar before (${similarity}% match): "${dejavu.match.summary || dejavu.match.errorText}"`, 'View Fix', 'Continue Anyway', 'Cancel');
            if (action === 'View Fix') {
                vscode.env.openExternal(vscode.Uri.parse(`${(0, auth_1.getApiUrl)()}/entries/${dejavu.match.id}`));
                return;
            }
            if (action === 'Cancel')
                return;
        ...
        vscode.window.withProgress({
            ...
            const result = await (0, api_1.saveEntry)({
                errorText,
                fixText,
                codeSnippet,
                context: contextStr
            ...
            if (result.success) {
                ...
                if (action === 'View Entry') {
                    vscode.env.openExternal(vscode.Uri.parse(`${(0, auth_1.getApiUrl)()}/entries/${result.entryId}`));
                }
            }
            else {
    ...
    const dejavuCmd = vscode.commands.registerCommand('debugdiary.checkDejavu', async () => {
        ...
        vscode.window.withProgress({
            ...
            const result = await (0, api_1.checkDejavu)(selectedText);
            if (result.match) {
                const similarity = Math.round((result.similarity || 0) * 100);
                const action = await vscode.window.showWarningMessage(`⚡ Déjà Vu! ${similarity}% match: "${result.match.summary || result.match.errorText}"\nFix: "${result.match.fixText.substring(0, 80)}..."`, 'View Full Entry', 'Dismiss');
                if (action === 'View Full Entry') {
                    vscode.env.openExternal(vscode.Uri.parse(`${(0, auth_1.getApiUrl)()}/entries/${result.match.id}`));
                }
            }
            else {
    ...
    const dashboardCmd = vscode.commands.registerCommand('debugdiary.openDashboard', () => {
        vscode.env.openExternal(vscode.Uri.parse((0, auth_1.getApiUrl)()));
    });
